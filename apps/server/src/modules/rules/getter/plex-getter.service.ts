  ...
  async get(
    ...
    try {
      ...
      switch (prop.name) {
        ...
        case 'lastPlayedAt': {
          // The item's own lastViewedAt field is admin-scoped: Plex metadata
          // endpoints return the calling account's data only, so shared-user
          // plays are invisible here. We therefore mirror the lastViewedAt
          // (id 7) strategy: seed the reduction from libItem.lastViewedAt so
          // partial admin plays not yet committed to history are preserved,
          // then let any newer history row from any user win.
          // getWatchHistory uses /status/sessions/history/all with the admin
          // token and returns every user's entries - the same endpoint that
          // lastViewedAt already relies on.
          const seenby = await this.plexApi.getWatchHistory(
            metadata.ratingKey,
            true,
            metadata.type,
