...
describe('PlexGetterService', () => {
  ...
  describe('exhaustive Plex rule property getters', () => {
    ...
    describe('lastPlayedAt (id 47)', () => {
      it('returns the item lastViewedAt when history is empty (id 47)', async () => {
        const lastViewedAt = new Date(1_730_000_000 * 1000);
        plexApi.getMetadata.mockResolvedValue(makeMetadata());
        plexApi.getWatchHistory.mockResolvedValue([]);
      ...
      it('now calls getWatchHistory to aggregate all-user plays (id 47)', async () => {
        plexApi.getMetadata.mockResolvedValue(makeMetadata());
        plexApi.getWatchHistory.mockResolvedValue([]);
      ...
      it('returns a shared-user history date that is newer than the admin item field (id 47)', async () => {
        const adminLastViewedAt = new Date(1_730_000_000 * 1000);
        // Shared user watched after admin
        const sharedUserViewedAt = 1_740_000_000;
        plexApi.getMetadata.mockResolvedValue(makeMetadata());
        plexApi.getWatchHistory.mockResolvedValue([
          makeWatchEntry({ viewedAt: sharedUserViewedAt, accountID: 2 }),
        ]);

        const result = await service.get(
          47,
          createMediaItem({ type: 'movie', lastViewedAt: adminLastViewedAt }),
          'movie',
          createRuleGroupDto({ dataType: 'movie' }),
        );

        expect(result).toEqual(new Date(sharedUserViewedAt * 1000));
      });

      it('prefers admin item lastViewedAt when it is newer than the history rows (id 47)', async () => {
        // Partial play by admin occurred after last completed view by any user
        const adminLastViewedAt = new Date(1_750_000_000 * 1000);
        plexApi.getMetadata.mockResolvedValue(makeMetadata());
        plexApi.getWatchHistory.mockResolvedValue([
          makeWatchEntry({ viewedAt: 1_730_000_000 }),
        ]);
        ...
        expect(result).toEqual(adminLastViewedAt);
      });

      it('returns null when no history and no item lastViewedAt (id 47)', async () => {
        plexApi.getMetadata.mockResolvedValue(makeMetadata());
        plexApi.getWatchHistory.mockResolvedValue([]);

        const result = await service.get(
          47,
          createMediaItem({ type: 'movie', lastViewedAt: undefined }),
          'movie',
          createRuleGroupDto({ dataType: 'movie' }),
        );
      ...
      it('returns the newest date across multiple history rows from different users (id 47)', async () => {
        plexApi.getMetadata.mockResolvedValue(makeMetadata());
        plexApi.getWatchHistory.mockResolvedValue([
          makeWatchEntry({ viewedAt: 1_700_000_000, accountID: 1 }),
          makeWatchEntry({ viewedAt: 1_740_000_000, accountID: 2 }),
          makeWatchEntry({ viewedAt: 1_720_000_000, accountID: 3 }),
        ]);
